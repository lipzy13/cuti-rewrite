...
const FormCuti = ({ kontrak }) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <label htmlFor="nomorKontrak" className="text-gray-500 font-medium">
        Nomor Kontrak
      </label>
      <input
        type="text"
      ...
      <div className="relative">
        {fields.map((tanggal, index) => (
          <div className="my-2" key={tanggal.id}>
        ...
        py-3 px-4 block w-full border-gray-200 shadow-sm rounded-s-lg text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none border"
                {...register(`tanggal[${index}]`, { required: true })}
                defaultValue={tanggal}
              ></input>
              <button
      ...
      <div>
        <label htmlFor="file_path" className="text-gray-500 font-medium">
          Upload Surat Cuti,
          <span>
            <button type="button" className="text-sky-700">
              Download surat
            </button>
          </span>
        </label>
